...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_SKIN_TONE_BLOCK)
			H.skin_tone = sanitize_inlist(H.skin_tone, GLOB.skin_tones_species[species_index])
			setblock(uni_identity, blocknumber, construct_block(GLOB.skin_tones_list.Find(H.skin_tone), GLOB.skin_tones.len))
...
/mob/living/carbon/human/updateappearance(icon_update=1, mutcolor_update=0, mutations_overlay_update=0)
	..()
	var/species_index = dna.species.limbs_id ? dna.species.limbs_id : dna.species.id
