...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_FACIALHAIRSTYLE_BLOCK] = construct_block(GLOB.facial_hairstyles_list.Find(H.facial_hairstyle), GLOB.facial_hairstyles_list.len)
		L[DNA_FACIAL_HAIR_COLOR_BLOCK] = sanitize_hexcolor(H.facial_hair_color)
		if(!GLOB.skin_tones_list.len)
			init_sprite_color_subtypes(/datum/sprite_color/skin_tone, GLOB.skin_tones_list, GLOB.skin_tones_list_species)
		L[DNA_SKIN_TONE_BLOCK] = construct_block(GLOB.skin_tones_list.Find(H.skin_tone), GLOB.skin_tones_list.len)
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_SKIN_TONE_BLOCK)
			H.skin_tone = sanitize_inlist(H.skin_tone, GLOB.skin_tones_list_species[H.dna.species.limbs_id])
			setblock(uni_identity, blocknumber, construct_block(GLOB.skin_tones_list.Find(H.skin_tone), GLOB.skin_tones_list.len))
...
/datum/dna/proc/initialize_dna(newblood_type, skip_index = FALSE)
	...
	if(!skip_index) //I hate this
		generate_dna_blocks()
	features = random_features(features, species)
