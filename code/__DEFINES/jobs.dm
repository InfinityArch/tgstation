
#define ENGSEC			(1<<0)

#define CAPTAIN			(1<<0)
#define HOS				(1<<1)
#define NT_REP			(1<<2)
#define BLUESHIELD		(1<<3)
#define WARDEN			(1<<4)
#define MAGISTRATE		(1<<5)
#define DETECTIVE		(1<<6)
#define OFFICER			(1<<7)
#define LAWYER			(1<<9)
#define CHIEF			(1<<10)
#define ENGINEER		(1<<11)
#define ATMOSTECH		(1<<12)
#define ROBOTICIST		(1<<13)
#define AI_JF			(1<<14)
#define CYBORG			(1<<15)

#define CORP 		    (1<<3)
//#define NT_REP        (1<<0)
//#define MAGISTRATE      (1<<1)
//#define BLUESHIELD      (1<<2)
//#define RD_JF			(1<<2)
//#define SCIENTIST       (1<<3)
//#define GENETICIST      (1<<4)
//#define VIROLOGIST      (1<<5)
//#define ARCHEOLOGIST    (1<<6)
//#define ROBOTICIST	    (1<<7)
//#define AI_JF		 	(1<<8)
//#define CYBORG          (1<<9)

//#define LOCALGOV        (1<<0)

//#define CAPTAIN         (1<<0)

//#define HOS				(1<<2)
//#define WARDEN			(1<<3)
//#define DETECTIVE		(1<<3)
//#define OFFICER			(1<<4)
//#define CHIEF			(1<<5)
//#define ENGINEER		(1<<6)
//#define ATMOSTECH		(1<<7)
//#define CMO_JF			(1<<8)
//#define DOCTOR			(1<<9)
//#define CHEMIST			(1<<10)

#define MEDSCI			(1<<1)

#define RD_JF			(1<<1)
#define SCIENTIST		(1<<1)
#define CHEMIST			(1<<2)
#define CMO_JF			(1<<3)
#define DOCTOR			(1<<4)
#define MEDIC			(1<<5)
#define GENETICIST		(1<<6)
#define VIROLOGIST		(1<<7)


#define CIVILIAN		(1<<2)

#define HOP				(1<<0)
#define BARTENDER		(1<<1)
#define BOTANIST		(1<<2)
#define COOK			(1<<3)
#define JANITOR			(1<<4)
#define CURATOR			(1<<5)
#define QUARTERMASTER	(1<<6)
#define CARGOTECH		(1<<7)
#define MINER			(1<<8)
#define CHAPLAIN		(1<<9)
#define BARBER          (1<<10)
#define CLOWN			(1<<11)
#define MIME			(1<<12)
#define ASSISTANT		(1<<13)

#define JOB_AVAILABLE 0
#define JOB_UNAVAILABLE_GENERIC 1
#define JOB_UNAVAILABLE_BANNED 2
#define JOB_UNAVAILABLE_PLAYTIME 3
#define JOB_UNAVAILABLE_ACCOUNTAGE 4
#define JOB_UNAVAILABLE_SLOTFULL 5

#define DEFAULT_RELIGION "Christianity"
#define DEFAULT_DEITY "Space Jesus"

#define JOB_DISPLAY_ORDER_DEFAULT 0

#define JOB_DISPLAY_ORDER_ASSISTANT 1
#define JOB_DISPLAY_ORDER_CAPTAIN 2
#define JOB_DISPLAY_ORDER_NT_REP  3
#define JOB_DISPLAY_ORDER_HEAD_OF_PERSONNEL 4
#define JOB_DISPLAY_ORDER_QUARTERMASTER 5
#define JOB_DISPLAY_ORDER_CARGO_TECHNICIAN 6
#define JOB_DISPLAY_ORDER_SHAFT_MINER 7
#define JOB_DISPLAY_ORDER_BARTENDER 8
#define JOB_DISPLAY_ORDER_COOK 9
#define JOB_DISPLAY_ORDER_BOTANIST 10
#define JOB_DISPLAY_ORDER_JANITOR 11
#define JOB_DISPLAY_ORDER_CLOWN 12
#define JOB_DISPLAY_ORDER_MIME 13
#define JOB_DISPLAY_ORDER_CURATOR 14
#define JOB_DISPLAY_ORDER_CHAPLAIN 15
#define JOB_DISPLAY_ORDER_CHIEF_ENGINEER 16
#define JOB_DISPLAY_ORDER_STATION_ENGINEER 17
#define JOB_DISPLAY_ORDER_ATMOSPHERIC_TECHNICIAN 18
#define JOB_DISPLAY_ORDER_CHIEF_MEDICAL_OFFICER 19
#define JOB_DISPLAY_ORDER_MEDICAL_DOCTOR 20
#define JOB_DISPLAY_ORDER_CHEMIST 21
#define JOB_DISPLAY_ORDER_GENETICIST 22
#define JOB_DISPLAY_ORDER_VIROLOGIST 23
#define JOB_DISPLAY_ORDER_RESEARCH_DIRECTOR 24
#define JOB_DISPLAY_ORDER_SCIENTIST 25
#define JOB_DISPLAY_ORDER_ROBOTICIST 26
#define JOB_DISPLAY_ORDER_ARCHEOLOGIST 27
#define JOB_DISPLAY_ORDER_HEAD_OF_SECURITY 28
#define JOB_DISPLAY_ORDER_WARDEN 29
#define JOB_DISPLAY_ORDER_MAGISTRATE 30
#define JOB_DISPLAY_ORDER_DETECTIVE 31
#define JOB_DISPLAY_ORDER_SECURITY_OFFICER 32
#define JOB_DISPLAY_ORDER_LAWYER 33
#define JOB_DISPLAY_ORDER_BLUESHIELD 34
#define JOB_DISPLAY_ORDER_AI 35
#define JOB_DISPLAY_ORDER_CYBORG 36
